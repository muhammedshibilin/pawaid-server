...
export class RecruiterAlertController {
    ...
    async acceptRescue(req:Request,res:Response){
        try {
            const {animalReportId,recruiterId} = req.body
            const response = await this.recruiterAlertService.acceptAlert(animalReportId,recruiterId)
            return res.status(200).json({status:200,message:'accepted successfully'})
        } catch (error) {
